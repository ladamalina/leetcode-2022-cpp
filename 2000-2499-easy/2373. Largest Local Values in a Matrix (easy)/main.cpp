...
class Solution {
    public:
    static std::vector<std::vector<int>> largestLocal(const std::vector<std::vector<int>>& g) {
        const auto n = static_cast<int>(g.size());
        std::vector<std::vector<int>> a(n-2, std::vector<int>(n-2));
        for (auto gi=1; gi<n-1; ++gi) {
            const auto ai = gi-1;
            for (auto gj=1; gj<n-1; ++gj) {
                const auto aj = gj-1;
                a[ai][aj] = std::max(a[ai][aj], g[gi-1][gj-1]);
                a[ai][aj] = std::max(a[ai][aj], g[gi-1][gj]);
                a[ai][aj] = std::max(a[ai][aj], g[gi-1][gj+1]);
                a[ai][aj] = std::max(a[ai][aj], g[gi][gj-1]);
                a[ai][aj] = std::max(a[ai][aj], g[gi][gj]);
                a[ai][aj] = std::max(a[ai][aj], g[gi][gj+1]);
                a[ai][aj] = std::max(a[ai][aj], g[gi+1][gj-1]);
                a[ai][aj] = std::max(a[ai][aj], g[gi+1][gj]);
                a[ai][aj] = std::max(a[ai][aj], g[gi+1][gj+1]);
