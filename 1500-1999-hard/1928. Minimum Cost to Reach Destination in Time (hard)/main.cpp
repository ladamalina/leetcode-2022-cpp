...
std::array<int, MAX_N> times, costs;
...
public:
  int minCost(const int max_tm, const vvi& edges, const vi& fees) {
    ...
    std::fill(times.begin(), times.begin() + n, INT_MAX);
    std::fill(costs.begin(), costs.begin() + n, INT_MAX);
    times[0] = 0;
    costs[0] = fees[0];
    
    std::priority_queue<std::pair<ii, int>, std::vector<std::pair<ii, int>>, std::greater<>> pq;
    pq.emplace(ii{fees[0], 0}, 0);
    while (!pq.empty()) {
      const auto v_sp = pq.top().F.F, v_tm = pq.top().F.S, v = pq.top().S;
      pq.pop();
