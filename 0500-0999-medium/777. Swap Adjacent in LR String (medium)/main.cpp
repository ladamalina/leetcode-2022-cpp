...
class Solution {
public:
    static bool canTransform(const std::string& s, const std::string& t) {
        ...
        const auto s_cnt_x = std::count(s.begin(), s.end(), 'X');
        const auto t_cnt_x = std::count(t.begin(), t.end(), 'X');
