...
std::array<int, MAX_N+1> trust_in, trust_out;

class Solution {
 public:
  static int findJudge(int n, const std::vector<std::vector<int>>& trust) {
    std::fill(trust_in.begin()+1, trust_in.begin()+n+1, 0);
    std::fill(trust_out.begin()+1, trust_out.begin()+n+1, 0);
    ...
    for (int i=1; i<=n; ++i) {
      if (trust_out[i] == 0 && trust_in[i] == n-1) {
        return i;
      }
    }
    return -1;
