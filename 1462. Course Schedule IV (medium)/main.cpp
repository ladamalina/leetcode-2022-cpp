...
class Solution {
public:
  vb checkIfPrerequisite(const int n, const vvi& prerequisites, const vvi& queries) {
    vvi g(n);
    ...
    std::fill(RNG(vis), -1);
    const std::function<bool(int,int,int)> dfs = [&](const int v, const int x, const int qi) {
      if (v == x)
        return true;
      if (vis[v] == qi)
        return false;
    ...
    vb ans(qn);
    FOR(i, 0, qn-1) {
      ans[i] = dfs(queries[i][0], queries[i][1], i);
    }
